...
# for all subjects
    ...
    subject_dirs = glob(os.path.join(app.args.bids_dir, 'sub-*'))
...
for subject_label in subjects_to_analyze:
    ...
    all_dwi_images = glob(os.path.join(app.args.bids_dir, label, '*dwi', '*_dwi.nii*'))
    ...
    dwi_preproc_file = os.path.join(subject_dir, 'dwi_biasc.mif')
    ...
    # DW gradient files
    grad_prefix = os.path.join(app.args.bids_dir, label, 'dwi', label + '_dwi')
    if not (os.path.isfile(grad_prefix + '.bval') and os.path.isfile(grad_prefix + '.bvec')):
        grad_prefix = os.path.join(app.args.bids_dir, 'dwi')
        if not (os.path.isfile(grad_prefix + '.bval') and os.path.isfile(grad_prefix + '.bvec')):
    ...
    dwi_mrtrix_file = os.path.join(app._tempDir, subject_label + 'dwi.mif')
    run.command('mrconvert ' + all_dwi_images[0] + grad_import_option + json_import_option + ' ' + dwi_mrtrix_file)
    dwi_biascorrect_file = os.path.join(app._tempDir, subject_label + 'dwi_biascorr.mif')
    run.command('dwibiascorrect' + ' -ants ' + dwi_mrtrix_file + ' ' + dwi_denoised_file)

    # Estimate WM, GM, CSF response functions
    run.command('dwi2response ' + app._nthreads + ' dhollander ' + dwi_preproc_file + ' ' + wm_response_file + ' '
                                                                                              + gm_response_file + ' '
...
if mean_voxel_size > min_voxel_size:
  run.command('mrresize -vox ' + str(min_voxel_size) + ' ' + input_to_csd + ' ' + os.path.join(app._tempDir, subject_label + 'dwi_upsampled.mif'))
  input_to_csd = os.path.join(app._tempDir, subject_label + 'dwi_upsampled.mif')
...
run.command('mtbin -independent ' + os.path.join(app._tempDir, subject_label + 'fod.mif') + ' ' + output_fod + ' ' +
                                   os.path.join(app._tempDir, subject_label + 'gm.mif')  + ' ' + output_gm  + ' ' +
                                   os.path.join(app._tempDir, subject_label + 'csf.mif') + ' ' + output_csf + app.force)
...
run.command('mrconvert ' + os.path.join(app._tempDir, 'tmp.mif') + ' -header_set made_from_subset true ' + fod_template + app.force)
else:
...
run.command('mrconvert ' + os.path.join(app._tempDir, 'tmp.mif') + ' -header_set made_from_subset false ' + fod_template + app.force)
